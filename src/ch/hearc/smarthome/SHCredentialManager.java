 ...
 * @author Horia Mut
 */
public class SHCredentialManager
{


	// Debugging
	private static final String			TAG					= "SHCredentialManager";
	...
	private static File SAVE_FILEPATH = new File(SHFileUtil.SMARTHOME_DIR.getAbsolutePath()
	...
	public static void saveCredential(String _username)
	{
		...
		if(SHFileUtil.isMediaMounted( ))
		{
			try
			{
				SHFileUtil.writeTextFile(SAVE_FILEPATH, textToSave, true);
			}
			catch(IOException e)
			{
				e.printStackTrace( );
	...
	public static void update( )
	{
		...
		if(SHFileUtil.isMediaMounted( ))
		{
			if(SHFileUtil.SMARTHOME_DIR.exists( ))
			{
			...
			else
			{
				...
				SHFileUtil.createTree(SHFileUtil.SMARTHOME_DIR);
				update( );
				return;
			}
		}
