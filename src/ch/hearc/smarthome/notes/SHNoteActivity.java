...
public class SHNoteActivity extends SHBluetoothActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSchedule = new SimpleAdapter(this.getBaseContext(), listItem,
				R.layout.notes_list, new String[] { "sujet", "date" },
				new int[] { R.id.sujet, R.id.date });
		mSchedule.notifyDataSetChanged();
		// On attribut à notre listView l'adapter que l'on vient de créer
		myListView.setAdapter(mSchedule);

		/*
		 * For debug map.put("sujet", "Souper"); map.put("detail",
		 * "Detail ici"); map.put("date", "01.02.2011"); listItem.add(map);
		 * 
		 * map.put("sujet", "Souper"); map.put("detail", "Detail ici");
		 * map.put("date", "01.02.2011"); listItem.add(map)
		 */

		// ask only for user's notes
		String dataToSend = protocol.generateDataToSend(getNotesUser,
				protocol.generate(CredentialManager.getActualUser()));
		write(dataToSend);
		...
		myListView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> a, View v, int position,
				...
				adb.setMessage("Post-it details : " + newLine
						+ map.get("detail") + newLine + newLine);
						...
						new AlertDialog.OnClickListener() {
							public void onClick(DialogInterface dialog,
								...
								try {
									...
									write(protocol.generateDataToSend(
											CredentialManager.getActualUser(),
											deletepost, delSujet));
									mSchedule.notifyDataSetChanged();
								} catch (Exception e) {
